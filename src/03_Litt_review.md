...
Best practices seem to be summarized by three main points. An efficient display should be self-explanatory, tailored to the audience and most importantly convey the key message. Moreover a good visualization is highly dependent on the task, the type of display is closely related to the phenomena considered, and crucial to understand in that perspective [@cbo].
...
. ! LATEXequation (see src_drafts_formula)where LATEX a_{ki}  is the number of cells assigned to category i, in map k. (k LATEXappartient 1,2), N the total number of cells in map and C means all categories.
